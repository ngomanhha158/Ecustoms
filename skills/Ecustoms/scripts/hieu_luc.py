...
NGAY_HL = re.compile(
    r"(?:có hiệu lực|hiệu lực thi hành)[^.;]{0,40}?(?:kể\s+)?từ ngày\s+"
...
def _cau(noi_dung):
    ...
    return re.split(r"(?<=[.;:])\s+(?=[-–A-ZĐ0-9\"“(])", re.sub(r"\s{2,}", " ", phang))
...
def quan_he(so_hieu_minh, noi_dung):
    ...
    for cau in _cau(noi_dung):
        ...
        for m in SO_HIEU.finditer(cau, vt, vt + 150):   # số hiệu phải ở sát động từ
            dich = m.group(0)
